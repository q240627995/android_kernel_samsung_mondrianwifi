...
struct timer_list freq_mode_timer_;
...
{
	if (timeout > 0)
		mod_timer(&freq_mode_timer_, jiffies + msecs_to_jiffies(timeout));
	tbl_select[0] = 2;
	tbl_select[1] = 3;
	tbl_select[2] = 4;
	tbl_select[3] = 4;
...
static void dbs_init_freq_map_table(struct cpufreq_policy *policy)
{
	...
	init_timer(&freq_mode_timer_);
	freq_mode_timer_.function = switch_mode_timer;
	freq_mode_timer_.data = 0;
...
int set_two_phase_freq_(int cpufreq)
{
	int i  = 0;
	for ( i = 0 ; i < NR_CPUS; i++)
...
void set_two_phase_freq_by_cpu_ ( int cpu_nr, int cpufreq){
	two_phase_freq_array[cpu_nr-1] = cpufreq;
}

int input_event_boosted_(void)
{
	unsigned long flags;
...
void set_input_event_min_freq_by_cpu_ ( int cpu_nr, int cpufreq){
